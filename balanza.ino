...
if(digitalRead(2)==LOW) {esta_contando = 1; guarda_peso = 0;}
if(digitalRead(3)==LOW) bandera_inicio = 0;
...
if(esta_contando == 1)  {                  
 ...
 if(millis()-anteriores >= 2000)
 {
  ...
  peso_actual = balanza.get_units(20);
  peso_mostrar = peso_actual*1000;
  //Serial.print(peso_mostrar,5); Serial.println(" gramos"); 
  anteriores = millis();
  if(esta_contando == 0)
  {   
  lcd.setCursor(0, 0);
